...
**Playable, and unfinished.** Three eras, 13 producers, 31 upgrades and the full
automation ladder are in. Prestige, the narrative log and offline progress are
not yet — see [`PLAN.md`](PLAN.md) for the design and what is left.
...
src/game/     engine.ts, rates.ts, purchase.ts, unlocks.ts, automation.ts — all pure
src/state/    store.ts, versioned localStorage persistence, types
src/ui/       layout, resource rail, swarm, tech, log, ticker, toast
scripts/      validate-data.ts (content gate), verify-ui.ts (Playwright)
tests/        decimal / engine / purchase / persistence / data
```
...
`npm run verify` writes `screenshots/` at both breakpoints. If the sandbox's
