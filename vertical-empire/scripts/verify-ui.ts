...
/**
 * Long enough for a slow machine, short enough to fail rather than hang.
 *
 * A selector that no longer matches — a button moved into a drawer, say — used
 * to stall the whole run at Playwright's default of thirty seconds a click and
 * leave half the screenshots stale from the previous run, which reads as "it
 * worked" right up until you check the timestamps.
 */
...
async function main(): Promise<void> {
  ...
  try {
    ...
    // Without this a failed selector leaves Chromium running and the script
    // never exits — which looks like a hang rather than the error it is.
    await browser.close();
  }
}
...
async function shoot(browser: Browser): Promise<void> {
  ...
  for (const viewport of VIEWPORTS) {
    ...
    await page.goto(`${URL}?hour=13`, { waitUntil: 'load' });
    ...
    await page.click('.art-collapse', { timeout: CLICK_TIMEOUT });
    ...
    if (viewport.name === 'phone') {
      ...
      // Offices live in the Work drawer, so the drawer is opened first — which
      // is what a player does, and keeps this honest about the taps involved.
      await page.click('.tab >> text=Work', { timeout: CLICK_TIMEOUT });
      await page.click('button[data-tool="build:office"]', { timeout: CLICK_TIMEOUT });
      ...
      await page.goto(`${URL}?hour=22`, { waitUntil: 'load' });
      await page.waitForTimeout(400);
      await page.click('.art-collapse', { timeout: CLICK_TIMEOUT });
